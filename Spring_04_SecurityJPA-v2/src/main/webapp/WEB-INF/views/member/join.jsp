...
<%
    ...
    GET 방식으로 데이터를 전송할 때는 인증(로그인 정보)과 관련된 어떤 기능을 수행할 수 없다
    ...
    form 으로 만든 입력 변수 중에 _csrf.token 값에 서버로부터 전달받은 실제 token 을 부착하는 코드를 만들어야 한다

    이러한 절차가 순수 html 로 만든 form 에서는 상당히 번거로운 일이 된다
    따라서 spring jsp 코드에서는 spring form taglibs 를 import 하고 form:form 방식으로 HTML 코드를 작성한다
