...
import com.csy.sec.config.Message;
...
public class LoginFailService implements AuthenticationFailureHandler {
    ...
    public void onAuthenticationFailure(HttpServletRequest request,
        ...
        // 어떤 문제가 발생하여 LoginFail 이 작동되었는지를 알기 위해 exception 정체 파악하기
        // 1. exception 의 클래스 이름을 문자열로 비교하여 정체 파악하기
        ...
        // 2. exception 객체가 어떤 클래스의 인스턴스인가를 알아보고 exception 의 정체 파악하기
        //      type 오류는 발생하지 않는다
        String error_msg = "";
        ...
        /**
         * /member/login URL 은 login.jsp 를 열기 위한 요청
         ...
         * 이유 : POST /login 을 수행하는 과정에서 forwarding 을 했기 때문에
         * 
         * 만약 redirect 를 했다면 GET 방식으로 호출이 된다
         */

        request.setAttribute("ERROR_MSG", error_msg);
        
        // /member/login URL 로 forwarding 을 수행하라
        // redirect(X) forwarding(O)
        // (* 포워딩 하는 이유 : 위에 에러메시지 같이 담고 가기 위해)
